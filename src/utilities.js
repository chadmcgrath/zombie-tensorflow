...
export function project(v, d, t) {
    return new Vec(v.x + d.x * t, v.y + d.y * t);
}

export function randomAngle(s) {
    var r = Math.random() * pi2;
    if (!s) s = 1;
    return new Vec(Math.cos(r) * s, Math.sin(r) * s);
}

export function fixAngle(d) {
    while (d < 0) d += pi2;
    while (d >= pi2) d -= pi2;
    return d;
}

export function createGrid(ctx, minX, maxX, minY, maxY, z) {
...
export function line_intersect(line1Start, line1End, line2Start, line2End) {
...
export const line_point_intersect = function (p1, p2, p0, rad) {
    var dx = p2.x - p1.x;
    var dy = p2.y - p1.y;
...
export function lineIntersectsSquare(lineStart, lineEnd, square) {
...
export const stuff_collide = (agent, p2, blocks, check_walls, check_items) => {
    var minres = false;
